...
if (isset($_POST['submit'])) {
  ...
  $stmt = $conn->prepare("INSERT INTO jiier_assignment (course_id,semester_id,title,last_date) VALUES (?,?,?,?)");
  $stmt->bind_param("ssss", $course_id,$semester_id,$title,$last_date);
  $stmt->execute() or die ($stmt->error);
  $assignment_id=$stmt->insert_id;  
  $file_name = $_FILES['question']['name'];
  if (trim($file_name) != "") {
      $ext = pathinfo($file_name, PATHINFO_EXTENSION);
      $file_name = $assignment_id . "." . $ext;
      $query = "update jiier_assignment set question = '" . $file_name . "' where id=$assignment_id";
